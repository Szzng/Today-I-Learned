...
- 1 ≤ number의 원소 ≤ 10
- number[i]는 need[i]의 수량을 의미하며, number의 원소의 합은 10입니다.
...
# 풀이
- solution1: out, new를 매번 체크하여 want의 값을 변경하는 방식
- solution2: 매번 10개씩 묶어서 새로운 want를 만들고, 이를 기존 want와 비교하는 방식
- want의 크기(m)가 작고, discount의 길이(num)가 클 경우, Solution 1이 Solution 2보다 성능이 더 좋을 가능성이 높음
- test에서도 Solution 1이 Solution 2보다 평균 2.7배 빠름
"""


def solution1(want, number, discount):
    need = {item: num for item, num in zip(want, number)}
    window = {item: 0 for item in want}
    count = 0
    ...
    if all(window[item] >= need[item] for item in want):
        count += 1
...
def solution2(want, number, discount):
    ...
    for i in range(len(discount) - 9):
        ...
        for j in range(i, i + 10):
            if discount[j] in need:
                products[discount[j]] = products.get(discount[j], 0) + 1
