...
# 입력 : record: ["Enter uid1234 Muzi", "Enter uid4567 Prodo","Leave uid1234","Enter uid1234 Prodo","Change uid4567 Ryan"]
...
def solution(record):
    ...
    for msg in record:
        ...
        if msg[0] == 'Enter':
            id_to_nick[msg[1]] = msg[2]
            actions.append([msg[1], '님이 들어왔습니다.'])

        elif msg[0] == 'Leave':
            actions.append([msg[1], '님이 나갔습니다.'])

        else:  # Change
            id_to_nick[msg[1]] = msg[2]

    return [''.join([id_to_nick[user_id], action]) for user_id, action in actions]
