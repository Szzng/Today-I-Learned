...
def solution(participant, completion):
    completed_counter = {}
    for c in completion:
        completed_counter[c] = completed_counter[c] + 1 if c in completed_counter else 0

    for p in participant:
        if p not in completed_counter:
            return p
        elif completed_counter[p] > 0:
            completed_counter[p] -= 1
        else:
            del completed_counter[p]

    return list(completed_counter.keys())[0]
