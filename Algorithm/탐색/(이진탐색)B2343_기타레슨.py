...
# 시간제한: 2초
# 시간복잡도: O(NlogX) (N: 강의 수, X: 강의 길이의 합 - 최장 강의 길이)
"""
import sys
...
while start <= end:
    ...
    # cnt<=M: cnt==M인 경우도 블루레이 크기를 더 줄일 수 있는지 확인하기 위해 end를 mid-1로 해줘야 함(최소 크기를 구하는 것이므로)
    if cnt <= M:  # 블루레이 개수가 M개 이하 -> 블루레이 크기를 더 줄이고 블루레이 개수를 늘려도 됨
        end = mid - 1
    else:  # 블루레이 개수가 M개 초과 -> 블루레이 크기를 더 늘리고 블루레이 개수를 줄여야 함
        start = mid + 1

# 이진탐색 종료 시점에 end에는 mid-1(마지막으로 시도했던 크기보다 1 작은 값)이 저장되어 있으므로, start를 출력해야 함
print(start)
