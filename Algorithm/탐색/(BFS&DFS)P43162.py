...
def bfs(computers, visited, start_computer):
    q = deque([start_computer])
    visited[start_computer] = True

    while q:
        curr_computer = q.popleft()

        for next_computer, linked in enumerate(computers[curr_computer]):
            if linked and not visited[next_computer]:
                q.append(next_computer)
                visited[next_computer] = True


def dfs(computers, visited, curr_computer):
    visited[curr_computer] = True

    for next_computer, linked in enumerate(computers[curr_computer]):
        if linked and not visited[next_computer]:
            dfs(computers, visited, next_computer)


def solution(num_nodes, computers):
